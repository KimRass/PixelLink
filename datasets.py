...
class ICDAR15Dataset(Dataset):
    def __init__(self, images_dir, labels_dir):
        ...
        self.all_labels = self.read_labels(labels_dir, 1000)
...
class PixelLinkIC15Dataset(ICDAR15Dataset):
    ...
    def test_data_transform(self, index):
        ...
        labels, img, size = ResizeImageWithLabel(labels, (512, 512), data=img)
        img = ZeroMeanImage(img, 123, 117, 104)
    ...
    def train_data_transform(self, index):
        ...
        rotate_rand = random.random() if True else 0
        crop_rand = random.random() if True else 0
        # rotate
        if rotate_rand > 0.5:
            labels, img, angle = RotateImageWithLabel(labels, data=img)
        # crop
        if crop_rand > 0.5:
            scale = 0.1 + random.random() * 0.9
            labels, img, img_range = CropImageWithLabel(labels, data=img, scale=scale)
    ...
    def label_to_mask_and_pixel_pos_weight2(label, img_size, version="2s", neighbors=8):
        ...
        pixel_weight = np.zeros(pixel_mask_size, dtype=np.float64)
        ...
        for bbox_index, bbox_mask in enumerate(bbox_masks):
            ...
            bbox_contours = find_contours(bbox_positive_pixel_mask)
    ...
    def label_to_mask_and_pixel_pos_weight(label, img_size, version="2s", neighbors=8):
        ...
        for i in range(label.shape[0]): # num of box
            if not ignore[i]:
                pixel_weight_tmp = np.zeros(pixel_mask_size, dtype=np.float64)
...
if __name__ == '__main__':
    start = time.time()
    dataset = PixelLinkIC15Dataset(
        images_dir="/Users/jongbeomkim/Documents/datasets/icdar2015/train_images/images/",
        labels_dir="/Users/jongbeomkim/Documents/datasets/icdar2015/train_images/ground_truth/"
    )
